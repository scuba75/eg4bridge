...
async function getInverterData(serialNumber){
  try{
    ...
    let r = await fetch(`http://${OPENDTU_HOST}/api/livedata/status?inv=${serialNumber}`)
...
module.exports = async(inverter_num, influxWrite, timeNow)=>{
  try{
    //return
    ...
    for(let i in MICRO_INVERTER_SN){
      ...
      if(!dataList?.inverters[inverter_num]) continue
      let data = await getInverterData(MICRO_INVERTER_SN[i])
      if(!data?.inverters || data?.inverters?.length == 0) continue
      let pv_energy_ac = roundValue((+(data.inverters[0]?.INV["0"]?.YieldDay?.v || 0)) / 1000, 1)
      if(pv_energy_ac == 0 || pv_energy_ac >= (dataList.inverters[inverter_num].pv_energy_ac || 0)){
        dataList.inverters[inverter_num].pv_energy_ac = pv_energy_ac
        pv_energy_total += pv_energy_ac
        await mqtt.sendSensorValue(`solar_inverter/${inverter_num}/pv_energy_ac/state`, pv_energy_ac)
        influxWrite('pv_energy_ac', `inverter_${inverter_num}`, pv_energy_ac, 'kWh', timeNow)
      }
    }
    if(dataList?.main){
      if(pv_energy_total == 0 || pv_energy_total >= (dataList.main.pv_energy_ac || 0)){
        dataList.main.pv_energy_ac = pv_energy_total
        await mqtt.sendSensorValue(`solar_inverter/main/pv_energy_ac/state`, pv_energy_total)
        influxWrite('pv_energy_ac', `main`, pv_energy_total, 'kWh', timeNow)
      }
    }
    */
  }catch(e){
