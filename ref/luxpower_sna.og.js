...
/**
 * LuxPower SNA Node.js client
 * - TCP translated-data protocol
 * - Polls READ_INPUT and READ_HOLD
 * - Supports WRITE_SINGLE queue
 * - Scans LAN /24 for dongle
 */


class LuxpowerSNA extends EventEmitter {
  // Protocol constants
  ...
  constructor(options = {}) {
    ...
    // ---- Text tables (optional) ----
    this.STATUS_TEXTS = new Array(193).fill(null);
    this.STATUS_TEXTS.splice(0, 0,
    ...
    this.BAT_STATUS_TEXTS = new Array(17).fill("");
    this.BAT_STATUS_TEXTS[0]  = "Charge Forbidden & Discharge Forbidden";
    this.BAT_STATUS_TEXTS[1]  = "Unknown";
    this.BAT_STATUS_TEXTS[2]  = "Charge Forbidden & Discharge Allowed";
    this.BAT_STATUS_TEXTS[3]  = "Charge Allowed & Discharge Allowed";
    this.BAT_STATUS_TEXTS[16] = "Charge Allowed & Discharge Forbidden";
  }
    ...
    switch (this.state) {
      ...
      case this.State.POLLING_INPUT: {
        ...
        if (this.bankIdx < 5) {
          ...
          this.emit('log', { level: 'info', msg: 'Input poll cycle complete.' });
          this.state = this.State.IDLE;
        }
        break;
      }
      ...
      case this.State.POLLING_HOLD: {
        if (this.bankIdx < 6) {
          ...
          this.emit('log', { level: 'info', msg: 'Hold poll cycle complete.' });
          this.initialHoldDone = true;
          this.lastHoldPollMs = now;
  ...
  processPacket(pkt) {
    ...
    if (tcpFn === LuxpowerSNA.LUX_TCP_HEARTBEAT) {
      this.emit('log', { level: 'debug', msg: 'Heartbeat – echoing back' });
      this.sendBytes(pkt);
      return;
    }

    if (tcpFn !== LuxpowerSNA.LUX_TCP_TRANSLATED_DATA) {
    ...
    if (devFn === LuxpowerSNA.LUX_FN_READ_INPUT) {
    ...
    if (devFn === LuxpowerSNA.LUX_FN_READ_HOLD) {
      if (df.length < 15) return;
      const vlen = df[14];
      const data = df.subarray(15, 15 + vlen);
      if (data.length !== vlen) return;
    ...
    if (devFn === LuxpowerSNA.LUX_FN_WRITE_SINGLE) {
  ...
  processReadHold(startReg, data) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (reg >= 240) break;
      this.holdRegs[reg] = data.readUInt16LE(i * 2);
    }
    this.emit('log', { level: 'debug', msg: `READ_HOLD reg=${startReg} count=${count} cached` });
    // In your C++ you notify at end of cycle; we do that too (hold poll complete).
  }
  */
  processReadHold(startReg, data) {
    const count = Math.floor(data.length / 2);
  ...
  /*
  // ---- READ_INPUT bank parsing (roughly mirrors your process_bank0..4_) ----
  processReadInput(startReg, data) {
    if (data.length < 80) {
      this.emit('log', { level: 'warn', msg: `READ_INPUT start=${startReg} too small (${data.length})` });
      return;
    }

    switch (startReg) {
      case 0:   return this.processBank0(data);
      case 40:  return this.processBank1(data);
      case 80:  return this.processBank2(data);
      case 120: return this.processBank3(data);
      case 160: return this.processBank4(data);
      default:
        this.emit('log', { level: 'warn', msg: `Unknown INPUT bank start_reg=${startReg}` });
    }
  }
  */
  processReadInput(startReg, data) {
  ...
  processHoldDerived() {
    ...
    const derived = {
      meta: {
        ...
        writes_pending: this.writeQueue.length
      },

      // Example bit exposures (no semantic assumption)
      bits: {
        reg21_bit0: (this.holdRegs[21] & 0x0001) !== 0,
        reg21_bit1: (this.holdRegs[21] & 0x0002) !== 0,
        reg21_bit2: (this.holdRegs[21] & 0x0004) !== 0
      },

      // Example numeric limits (safe scale, no meaning assumed)
      limits: {
        reg83_raw: this.holdRegs[83],
        reg83_scaled: this.holdRegs[83] / 10,
        reg83_disabled: this.holdRegs[83] === 101
      },

      // Time‑encoded registers (HH:MM)
      times: {
        ac_chg_start_0: decodeHHMM(this.holdRegs[68]),
        ac_chg_end_0: decodeHHMM(this.holdRegs[69]),
        ac_first_start_0: decodeHHMM(this.holdRegs[152]),
        ac_first_end_0: decodeHHMM(this.holdRegs[153]),
        ac_chg_start_0_raw: this.holdRegs[68]
      }
    };

    this.emit('hold_derived', derived);
  }
  // --------------------------
  ...
  processBank0(buf) {
    ...
    this.emit('bank0', payload);
    this.emit('sensor', { name: 'status_text', value: statusText });
  }
  ...
  processBank2(buf) {
    ...
    this.emit('sensor', { name: 'battery_status_text', value: batStatusText });
  }
  ...
  processBank4(buf) {
    ...
    this.emit('bank4', { p_load_ongrid, e_load_day, e_load_all_l });
  }
  ...
  buildHeader(dataLength) {
    ...
    pkt[6] = 0x01;
    pkt[7] = LuxpowerSNA.LUX_TCP_TRANSLATED_DATA;
  ...
  sendReadInput(startReg, count = 40) {
    ...
    df[0] = LuxpowerSNA.LUX_ACTION_WRITE;
    df[1] = LuxpowerSNA.LUX_FN_READ_INPUT;
    df.fill(0x00, 2, 12);
  ...
  sendReadHold(startReg, count = 40) {
    ...
    df[0] = LuxpowerSNA.LUX_ACTION_WRITE;
    df[1] = LuxpowerSNA.LUX_FN_READ_HOLD;
    df.fill(0x00, 2, 12);
    df.write(this.inverterSerial, 2, 10, 'ascii');
    df.writeUInt16LE(startReg, 12);
    df.writeUInt16LE(count, 14);
  ...
  sendWriteSingle(reg, value) {
    ...
    df[0] = LuxpowerSNA.LUX_ACTION_WRITE;
    df[1] = LuxpowerSNA.LUX_FN_WRITE_SINGLE;
    df.fill(0x00, 2, 12);
    df.write(this.inverterSerial, 2, 10, 'ascii');
